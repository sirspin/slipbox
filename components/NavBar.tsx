        ...
        if (
          ...
          return (
            <Link key={item.path} href={`/${item.name.replace(".md", "")}`}>
              ...
              >
                <span className={left ? "mr-auto" : "right"}>
    ...
    if (
      ...
      return (
        <div key={item.name} className="">
          ...
          >
            <a className={classNames("flex", left ? "mr-auto" : "right")}>
              {left && (
                <span
                  className={classNames(
                    "inset-y-0 left-0 flex items-center",
                    openFolders.filter((f) => f === item.path).length === 0
                      ? "transition duration-150 ease-out opacity-100 rotate-[270deg]"
                      : "transition duration-200 ease-out opacity-100 rotate-[360deg]"
                  )}
                >
                  <svg
              ...
              <span className={classNames("h-full", left ? "ml-1" : "mr-1")}>
                {item.name}
              </span>
              {!left && (
                <span
                  className={classNames(
                    "inset-y-0 left-0 flex items-center",
                    openFolders.filter((f) => f === item.path).length === 0
                      ? "transition duration-150 ease-out opacity-100 rotate-90"
                      : "transition duration-200 ease-out opacity-100 rotate-0"
                  )}
                >
                  <svg
                    className="h-4 w-4 m-auto arrow"
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 320 512"
                  >
                    <path d="M310.6 246.6l-127.1 128C176.4 380.9 168.2 384 160 384s-16.38-3.125-22.63-9.375l-127.1-128C.2244 237.5-2.516 223.7 2.438 211.8S19.07 192 32 192h255.1c12.94 0 24.62 7.781 29.58 19.75S319.8 237.5 310.6 246.6z" />
          ...
          <Transition
            show={openFolders.filter((f) => f === item.path).length !== 0}
          ...
          >
            <div
              ...
              className={classNames(left ? "mr-2" : "ml-2")}
            >
  ...
  return (
    <>
      ...
      <div id="markbase-navbar" className="md:w-1/4">
        ...
        <div className="hidden md:block text-left h-screen sticky top-0 py-4 md:py-16 ">
          <div className="flex-1 flex flex-col min-h-0 h-full border-r-2 dark:border-neutral-600 border-neutral-200 pr-0 md:pr-4">
            <div className="flex items-center flex-shrink-0 px-6 text-2xl font-bold mb-2"  style={{paddingLeft: 2 + 'em'}}>
              <Link href="/">
                <h2 className="dark:hover:text-indigo-500 hover:text-indigo-800 text-indigo-600 dark:text-indigo-400 text-left cursor-pointer">
            ...
            <div className="">
              ...
              >
                <nav className="flex-1 p-2" style={{paddingLeft: 3 + 'em'}}>{getNavigation()}</nav>
              </div>
            </div>
          </div>
        </div>
